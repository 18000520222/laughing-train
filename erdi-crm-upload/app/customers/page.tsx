    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <header className="mb-8 flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">管理 ERDI TECH 所有的 ED 编号客户</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="p-4 font-bold text-gray-600 text-sm">邮箱</th>
          ...
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                <td className="p-4"><span className="bg-blue-50 text-blue-700 px-2 py-1 rounded font-mono text-sm font-bold">{c.customerCode || '未分配'}</span></td>
                <td className="p-4 font-bold text-gray-800">{c.name}</td>
                <td className="p-4 text-gray-600">{c.contacts[0]?.firstName || '-'}</td>
                <td className="p-4 text-gray-500 text-sm">{c.email || c.contacts[0]?.email}</td>
                <td className="p-4"><span className="bg-green-50 text-green-700 px-2 py-1 rounded text-xs font-bold">{c.opportunities.length} 个商机</span></td>
                <td className="p-4">
                  <Link href={`/opportunity?companyId=${c.id}`} className="text-blue-600 hover:underline text-sm font-medium">查看详情</Link>
                </td>
              </tr>
            ))}
          </tbody>
