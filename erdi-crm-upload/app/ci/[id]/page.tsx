...
export default async function CommercialInvoice({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasLockedData = opp.lockedCiData && typeof opp.lockedCiData === 'object';

  if (isClosedWon && hasLockedData) {
    invoiceData = opp.lockedCiData as any;
  } else {
    ...
    if (isClosedWon && !hasLockedData) {
      invoiceData.isFrozen = true;
      await prisma.opportunity.update({
        where: { id: opp.id },
        data: { lockedCiData: invoiceData }
      });
    }
  }
      ...
      <div className="bg-white w-[210mm] min-h-[297mm] shadow-2xl print:shadow-none p-12 relative">
        ...
        <header className="border-b-2 border-gray-800 pb-6 mb-8 flex justify-between items-end relative z-0">
          ...
          <div className="text-right">
            <h2 className="text-3xl font-light text-indigo-800 tracking-widest">COMMERCIAL INVOICE</h2>
            <p className="text-gray-600 mt-2 font-mono">No. {invoiceData.ciNumber}</p>
            <p className="text-gray-500 text-sm">Date: {invoiceData.date}</p>
          </div>
        </header>
        ...
        <div className="flex justify-between mb-10 text-sm relative z-0">
          <div className="w-1/2 pr-4">
            ...
            <p className="font-bold text-gray-700">{invoiceData.companyName}</p>
            <p className="text-gray-600 mt-1">Email: {invoiceData.email}</p>
          </div>
        ...
        <table className="w-full mb-10 border-collapse relative z-0">
          ...
          <tbody>
            <tr className="text-sm">
              ...
              <td className="py-3 px-3 border-b border-gray-200 font-bold text-gray-800">{invoiceData.description}</td>
              <td className="py-3 px-3 border-b border-gray-200 text-gray-600">{invoiceData.hsCode || '-'}</td>
          ...
          <tfoot>
            <tr>
              ...
              <td className="py-3 px-3 text-right font-bold text-xl text-indigo-800 border-t-2 border-gray-800">${invoiceData.amountUSD}</td>
            </tr>
          </tfoot>
        </table>
