...
export default async function PackingList({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isClosedWon && hasLockedData) {
    ...
    if (isClosedWon && !hasLockedData) {
      plData.isFrozen = true;
      await prisma.opportunity.update({
        where: { id: opp.id },
        data: { lockedPlData: plData }
      });
    }
  }
      ...
      <div className="bg-white w-[210mm] min-h-[297mm] shadow-2xl print:shadow-none p-12 relative">
        ...
        <header className="border-b-2 border-gray-800 pb-6 mb-8 flex justify-between items-end relative z-0">
          ...
          <div className="text-right">
            <h2 className="text-3xl font-light text-indigo-800 tracking-widest">PACKING LIST</h2>
            <p className="text-gray-600 mt-2 font-mono">No. {plData.plNumber}</p>
            <p className="text-gray-500 text-sm">Date: {plData.date}</p>
          </div>
        </header>
        ...
        <div className="flex justify-between mb-10 text-sm relative z-0">
          <div className="w-1/2 pr-4">
            ...
            <p className="font-bold text-gray-700">{plData.companyName}</p>
            <p className="text-gray-600 mt-1">Email: {plData.email}</p>
          </div>
