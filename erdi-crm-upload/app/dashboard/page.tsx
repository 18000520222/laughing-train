      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-100/50 rounded-xl p-4 border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full">
              {opps.filter(o => o.stage === 'SPEC_CONFIRMING').length}
            </span>
          </div>
          <div>
            {opps.filter(o => o.stage === 'SPEC_CONFIRMING').map(renderCard)}
            {opps.filter(o => o.stage === 'SPEC_CONFIRMING').length === 0 && (
              <div className="text-center py-10 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">暂无数据</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-100/50 rounded-xl p-4 border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full">
              {opps.filter(o => o.stage === 'SAMPLE_TESTING').length}
            </span>
          </div>
          <div>
            {/* 就是这句代码，让第二列活过来了！ */}
            {opps.filter(o => o.stage === 'SAMPLE_TESTING').map(renderCard)}
            {opps.filter(o => o.stage === 'SAMPLE_TESTING').length === 0 && (
              <div className="text-center py-10 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">暂无商机拖拽至此</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-100/50 rounded-xl p-4 border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full">
              {opps.filter(o => o.stage === 'CLOSED_WON').length}
            </span>
          </div>
          <div>
            {/* 同样，让第三列也活过来！ */}
            ...
            {opps.filter(o => o.stage === 'CLOSED_WON').length === 0 && (
              <div className="text-center py-10 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">暂无成交记录</div>
            )}
          </div>
        </div>
