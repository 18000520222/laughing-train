    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <header className="mb-8 flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-6">
          ...
          <Link href="/attendance" className="flex items-center gap-2 bg-pink-50 text-pink-700 hover:bg-pink-100 px-4 py-2 rounded-lg font-bold transition-colors border border-pink-100 shadow-sm">
            📅 出差/请假
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-100/50 rounded-xl p-4 border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full">{opps.filter(o => o.stage === 'NEGOTIATING').length}</span>
          </div>
          <div>
            {opps.filter(o => o.stage === 'NEGOTIATING').map(renderCard)}
            {opps.filter(o => o.stage === 'NEGOTIATING').length === 0 && <div className="text-center py-10 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">暂无数据</div>}
          </div>
        </div>
        <div className="bg-gray-100/50 rounded-xl p-4 border border-gray-200">
          <div className="flex justify-between items-center mb-4">
