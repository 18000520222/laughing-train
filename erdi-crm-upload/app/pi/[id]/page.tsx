...
export default async function PIDocument({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isClosedWon && hasLockedData) {
    // 已经冻结过了，直接读取不可篡改的快照数据！
    invoiceData = opp.lockedPiData as any;
  } else {
    // 还没冻结（还在跟单中，或者刚改为已成单但还没生成快照）
    ...
    invoiceData = {
      piNumber: `PI-${new Date().getFullYear()}${new Date().getMonth()+1}-${shortId}`,
    ...
    // 如果刚变成已成单，这是第一次打开它，赶紧拍下快照并存进数据库！
    if (isClosedWon && !hasLockedData) {
      invoiceData.isFrozen = true;
      await prisma.opportunity.update({
        where: { id: opp.id },
        data: { lockedPiData: invoiceData }
      });
    }
  }
  // ==== 核心冻结逻辑结束 ==== //
      ...
      <div className="bg-white w-[210mm] min-h-[297mm] shadow-2xl print:shadow-none p-12 relative">
        ...
        <header className="border-b-2 border-gray-800 pb-6 mb-8 flex justify-between items-end relative z-0">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-600 mt-2 font-mono">No. {invoiceData.piNumber}</p>
