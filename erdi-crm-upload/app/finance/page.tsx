...
export default async function FinanceDashboard() {
  ...
  const opps = await prisma.opportunity.findMany({
    where: { stage: { in: ['CLOSED_WON', 'SAMPLE_TESTING'] } },
    orderBy: { updatedAt: 'desc' }
  });

  const totalRevenue = opps.filter(o => o.stage === 'CLOSED_WON').reduce((sum, o) => sum + (o.amount || 0), 0);
        ...
        <header className="mb-8 flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="flex items-center gap-8">
            <div className="text-right">
              <p className="text-sm text-gray-500">已成单总营收</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm text-gray-800">
              {opps.length === 0 && (
                <tr><td colSpan={6} className="p-10 text-center text-gray-400">暂无财务数据记录</td></tr>
              )}
              {opps.map(opp => (
                <tr key={opp.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 font-medium text-blue-700">{opp.companyId || '未填写'}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right font-bold text-gray-800">${opp.amount || 0}</td>
