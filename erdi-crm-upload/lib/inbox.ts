...
function channelLabel(c: ChannelType): string {
  return (
    { WHATSAPP: 'WhatsApp', ALIBABA: '阿里国际站', AMAZON: '亚马逊', SHOPEE: '虾皮', FACEBOOK: 'Facebook', INSTAGRAM: 'Instagram', LINKEDIN: 'LinkedIn', EMAIL: '邮件', SALESMARTLY: 'SaleSmartly' } as Record<
      ChannelType,
      string
    >
  )[c];
...
function notificationType(c: ChannelType): string {
  if (c === 'FACEBOOK' || c === 'INSTAGRAM' || c === 'LINKEDIN' || c === 'SALESMARTLY') return 'SOCIAL';
